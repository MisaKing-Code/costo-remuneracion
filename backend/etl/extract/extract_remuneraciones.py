...
from backend.config.settings import ETL_CONFIG, REMUNERACIONES_COLUMNS, VALID_SOCIEDADES
...
def _read_file(path):
    config = ETL_CONFIG["remuneraciones"]
...
def extract_remuneraciones():
    ...
    for path in sorted(raw_dir.rglob(config["file_pattern"])):
        if path.name.startswith("~$"):
            continue
        LOGGER.info("Extrayendo remuneraciones: %s", path)
        frame = _read_file(path)
        missing = [column for column in REMUNERACIONES_COLUMNS if column not in frame.columns]
        if missing:
            LOGGER.warning("Columnas faltantes en %s: %s", path.name, missing)
        frames.append(frame)

    if not frames:
        LOGGER.warning("No se encontraron archivos de remuneraciones en %s", raw_dir)
        return pd.DataFrame()
