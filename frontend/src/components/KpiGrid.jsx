import { Banknote, Building2, CircleDollarSign, Landmark, MapPinned, UsersRound } from "lucide-react";
import { formatCompactCurrency } from "../utils/formatters";

function KpiCard({ icon: Icon, label, value, secondary }) {
  return (
    <article className="panel-soft group p-4 transition duration-300 hover:-translate-y-0.5 hover:border-flame-400/35">
      <div className="flex items-start justify-between gap-3">
        <div className="rounded-lg border border-flame-400/20 bg-flame-500/12 p-2 text-flame-300">
          <Icon size={17} strokeWidth={2.4} />
        </div>
      </div>
      <p className="tiny-label mt-4">{label}</p>
      <p className="metric-value mt-1">{value}</p>
      <p className="mt-1 text-[11px] font-semibold text-stone-400">{secondary}</p>
    </article>
  );
}

export default function KpiGrid({ stats }) {
  const workerMetric = stats.workerMetric;
  const workers = workerMetric?.value ?? stats.workers;
  const workerSubtitle = workerMetric?.subtitleTrabajadores ?? "Unicos filtrados";

  return (
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
      <KpiCard
        icon={CircleDollarSign}
        label="Costo Remuneracional"
        value={formatCompactCurrency(stats.totalCost)}
        secondary="Costo total filtrado"
      />
      <KpiCard
        icon={Banknote}
        label="Total Haberes"
        value={formatCompactCurrency(stats.totalHaberes)}
        secondary="Haberes filtrados"
      />
      <KpiCard
        icon={Landmark}
        label="Aportes Empresa"
        value={formatCompactCurrency(stats.employerContributions)}
        secondary="Leyes sociales y aportes"
      />
      <KpiCard icon={UsersRound} label="Trabajadores" value={workers} secondary={workerSubtitle} />
      <KpiCard icon={Building2} label="Empresas" value={stats.companies} secondary="Sociedades filtradas" />
      <KpiCard icon={MapPinned} label="Centros" value={stats.businessCenters} secondary="Centros filtrados" />
    </section>
  );
}
